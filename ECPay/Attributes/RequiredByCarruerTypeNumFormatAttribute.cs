    ...
    public class RequiredByCarruerTypeNumFormatAttribute : RequiredAttribute
    {
        ...
        public override bool IsValid(object value)
        {
            ...
            if (oPropertyName.Equals("CarruerNum"))
            {
                ...
                if (oNeedCheckedValue.Equals(ECarruerType.NaturalPersonEvidence) && !Regex.IsMatch(Convert.ToString(oPropertyValue), @"^[A-Za-z]{2}[0-9]{14}$"))
                {
                    return false;
                }
                if (oNeedCheckedValue.Equals(ECarruerType.PhoneBarcode) && !Regex.IsMatch(Convert.ToString(oPropertyValue), @"^/[+-. 0-9a-zA-Z]{7}$"))
                {
                    return false;
                }
            }
