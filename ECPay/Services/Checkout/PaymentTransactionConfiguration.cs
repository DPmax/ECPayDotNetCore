...
namespace ECPay.Services.Checkout
{
    ...
    public class PaymentTransactionConfiguration : IPaymentTransactionConfiguration
    {
        ...
        public IPaymentConfiguration UseMethod(EPaymentMethod? method, EPaymentSubMethod? sub = null, EPaymentIgnoreMethod? ignore = null)
        {
            ...
            switch (method.Value)
            {
                case EPaymentMethod.ALL:
                    if (sub.HasValue)
                        throw new ArgumentException("Sub method is not allowed when PaymentMethod is set to ALL.", nameof(sub));
                ...
                case EPaymentMethod.Union:
                    if (sub.HasValue)
                        throw new ArgumentException("Sub method is not allowed when PaymentMethod is set to Union.", nameof(sub));
                    if (ignore.HasValue)
                        throw new ArgumentException("Ignore method is not allowed when PaymentMethod is not set to ALL.", nameof(ignore));
                ...
                case EPaymentMethod.WebATM:
                    ...
                    _payment.ChoosePayment = method.Value.ToString();
                    if (sub.HasValue)
                        _payment.ChooseSubPayment = sub.Value.ToString();
                    break;
                default:
        ...
        {
            if (field1 != null)
                _payment.CustomField1 = field1;
            if (field2 != null)
                _payment.CustomField2 = field2;
            if (field3 != null)
                _payment.CustomField3 = field3;
            if (field4 != null)
                _payment.CustomField4 = field4;

            return _configuration;
        }

        public IPaymentConfiguration WithItems(IEnumerable<ITestCheckoutItem> items, string url = null, int? amount = null)
        {
        ...
        private int CalculateTotalAmount(IEnumerable<ITestCheckoutItem> items)
        {
            var amount = 0;
        ...
        private string GenerateItemName(IEnumerable<ITestCheckoutItem> items)
        {
